...
export const PublicSite: React.FC<PublicSiteProps> = ({ content, onAdminClick }) => {
    ...
    const renderLogo = (isDark = false) => {
        ...
        return (
            <span className={`text-2xl font-display font-black tracking-tighter ${isDark ? 'text-white' : 'text-corporate-900'}`}>
                {firstPart}<span className="text-enea-accent">{rest ? ` ${rest}` : ''}</span>
            </span>
        );
    };
        ...
        <div className="min-h-screen bg-corporate-50 font-sans text-corporate-900 selection:bg-enea-accent selection:text-white">
            ...
            <nav
                className={`fixed w-full z-50 transition-all duration-500 ${scrolled
                    ? 'glass-card shadow-2xl shadow-corporate-900/5 py-4'
                    : 'bg-transparent py-8'
                    }`}
            >
                    ...
                    <div className="flex justify-between items-center">
                        ...
                        >
                            <div className="flex flex-col">
                                {renderLogo(!scrolled)}
                                <div className={`text-[9px] font-black uppercase tracking-[0.4em] mt-0.5 transition-colors ${scrolled ? 'text-corporate-400' : 'text-white/60'}`}>
                        ...
                        <div className="hidden md:flex items-center gap-10">
                            ...
                            <div className="pl-6 border-l border-white/10 flex items-center gap-6">
                                <button
                                    ...
                                    className={`p-2.5 transition-all ${scrolled ? 'bg-corporate-100 text-corporate-400 hover:text-corporate-900' : 'bg-white/5 text-white/60 hover:text-white hover:bg-white/10'
                                        }`}
                ...
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
                    <div className="grid lg:grid-cols-12 gap-20 mb-24">
                        <div className="lg:col-span-5 space-y-8">
                            ...
                            <p className="max-w-md leading-relaxed text-corporate-500 text-lg font-light">
                                Leader panafricain spécialisé dans la conception, le déploiement et la maintenance d'infrastructures technologiques critiques.
                            </p>
                        ...
                        <div className="lg:col-span-3">
                            <h4 className="text-white font-display font-bold uppercase tracking-[0.3em] mb-10 text-[11px]">Exploration</h4>
                            <ul className="space-y-6">
                                {navLinks.map(link => (
                                    <li key={link.id}>
                                        <button onClick={() => scrollToSection(link.id)} className="text-corporate-500 hover:text-enea-accent transition-all text-sm font-medium">
                                            {link.label}
                                        </button>
                                    </li>
                                ))}
                        ...
                        <div className="lg:col-span-4">
                            <h4 className="text-white font-display font-bold uppercase tracking-[0.3em] mb-10 text-[11px]">Conformité & Légal</h4>
                            <ul className="space-y-6">
                                <li><a href="#" className="text-corporate-500 hover:text-enea-accent transition-all text-sm font-medium">Mentions Légales & Gouvernance</a></li>
                                <li><a href="#" className="text-corporate-500 hover:text-enea-accent transition-all text-sm font-medium">Confidentialité des Données (GDPR)</a></li>
                                <li><a href="#" className="text-corporate-500 hover:text-enea-accent transition-all text-sm font-medium">Éthique & Code de Conduite</a></li>
                                <li><a href="#" className="text-corporate-500 hover:text-enea-accent transition-all text-sm font-medium">Support Technique 24/7</a></li>
                            </ul>
                        </div>
                    </div>
                    ...
                    <div className="pt-12 border-t border-white/5 flex flex-col md:flex-row justify-between items-center gap-8">
                        <div className="flex flex-col md:flex-row items-center gap-4 text-xs font-medium text-corporate-600">
                            <p>&copy; {new Date().getFullYear()} {content.entityName} Group. All assets secured.</p>
                        </div>
                        <div className="flex items-center gap-4 bg-white/5 px-6 py-2.5 border border-white/5">
                            <div className="flex items-center gap-2">
                                ...
                                <span className="text-[10px] font-black uppercase tracking-widest text-corporate-300">Systèmes Critiques: OK</span>
                            </div>
                            <div className="w-px h-4 bg-white/10 mx-2"></div>
                            <span className="text-[10px] font-black uppercase tracking-widest text-corporate-500">v2.4.0-stable</span>
                        </div>
                    </div>
                </div>
            </footer>
...
const SocialLink = ({ icon }: { icon: any }) => (
    <a href="#" className="w-10 h-10 bg-white/5 border border-white/5 flex items-center justify-center hover:bg-enea-accent hover:border-enea-accent hover:text-white transition-all duration-300">
        {icon}
    </a>
);
