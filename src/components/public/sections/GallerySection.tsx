...
export const GallerySection: React.FC<GallerySectionProps> = ({ realizations }) => {
  ...
  const categories = [
    'Tous',
    ...Array.from(new Set(realizations.gallery.map((img) => img.category))),
  ];
  ...
  return (
    <section
      id="realisations"
      className="py-32 bg-white text-corporate-900 overflow-hidden relative"
    >
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="flex flex-wrap gap-0 bg-corporate-50 p-0 border border-corporate-100">
            {categories.map((cat) => (
              <button
                ...
                className={`px-8 py-4 text-xs font-black tracking-widest uppercase transition-all duration-500 ${
                  activeFilter === cat
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 auto-rows-[350px]">
          {filteredGallery.map((item, idx) => (
            <div
              key={idx}
              className={`group relative overflow-hidden cursor-pointer shadow-2xl shadow-corporate-900/5 bg-corporate-100 transition-all duration-700 ${
          ...
          <div className="relative z-10 flex flex-col items-center">
            ...
            <div className="flex flex-wrap justify-center gap-10 md:gap-20 items-center grayscale opacity-60 hover:grayscale-0 hover:opacity-100 transition-all duration-700">
              {['ORANGE', 'MTN', 'MOOV', 'HUAWEI', 'ERICSSON', 'NOKIA'].map((client) => (
                <span
                  key={client}
                  className="text-2xl md:text-3xl font-display font-black tracking-tighter text-corporate-200"
                >
                  {client}
                </span>
              ))}
      ...
      {selectedImage && (
        <div
          className="fixed inset-0 z-[100] bg-corporate-950/95 backdrop-blur-2xl flex items-center justify-center p-6 lg:p-12 animate-in fade-in duration-500 transition-all"
        ...
        >
          ...
          <div
            className="max-w-5xl w-full flex flex-col items-center animate-in zoom-in duration-500 ease-out"
          ...
          >
            <div className="relative w-full aspect-video overflow-hidden shadow-[0_0_80px_rgba(0,0,0,0.5)] border border-white/5 bg-corporate-900">
              <img
                src={
                  ...
                  `https://images.unsplash.com/photo-1544197150-b99a580bbc7c?auto=format&fit=crop&w=1200&q=80`
                }
            ...
            <div className="mt-10 flex flex-col items-center max-w-2xl">
              <span className="text-enea-accent font-black uppercase tracking-[0.3em] text-[10px] mb-4 bg-blue-500/10 px-4 py-1.5">
                {selectedImage.category}
              </span>
              <h3 className="text-3xl md:text-4xl font-display font-bold text-white text-center leading-tight uppercase tracking-tight">
