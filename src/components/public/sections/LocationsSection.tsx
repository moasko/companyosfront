...
export const LocationsSection: React.FC<LocationsSectionProps> = ({ locations, currentCountry }) => {
    return (
            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
                <div className="flex flex-col items-center text-center mb-20">
                    ...
                    <p className="mt-6 text-corporate-500 text-lg font-light max-w-2xl">
                        Notre réseau s'étend sur les hubs les plus dynamiques du continent pour assurer une connectivité sans faille.
                    </p>
                </div>
                            ...
                            >
                                ...
                                <div className="relative z-10 flex flex-col h-full">
                                    <div className="flex items-start justify-between mb-8">
                                        <div className="text-5xl transform group-hover:scale-110 group-hover:-rotate-6 transition-all duration-500 drop-shadow-sm">{loc.flag}</div>
                                        <div className={`px-3 py-1 rounded-none text-[10px] font-black uppercase tracking-widest ${isActive ? 'bg-enea-accent text-white' : 'bg-corporate-50 text-corporate-400'
                                            }`}>
                                            Active
                                        </div>
                                    </div>
                                    ...
                                    <div>
                                        <h4 className={`text-2xl font-display font-bold mb-2 ${isActive ? 'text-white' : 'text-corporate-900'}`}>
                                            {loc.country}
                                        </h4>
                                        <div className="flex items-center gap-2">
                                            <div className={`w-1.5 h-1.5 ${isActive ? 'bg-green-400' : 'bg-green-600'}`}></div>
                                            <p className={`text-[11px] uppercase tracking-widest font-black ${isActive ? 'text-corporate-400' : 'text-corporate-500'}`}>
                                                {loc.status}
                                            </p>
                                        </div>
                                    </div>

                                    <div className={`mt-10 pt-6 border-t ${isActive ? 'border-white/10' : 'border-corporate-100'}`}>
                                        <p className={`text-[10px] font-black uppercase tracking-widest ${isActive ? 'text-corporate-500' : 'text-corporate-400'}`}>
