...
import { Search, Command, ArrowRight, Package, Calculator, Users, Target, CheckSquare, Settings, Home, Globe } from 'lucide-react';
...
const NAVIGATION_ITEMS = [
    ...
    { id: 'nav-stock', label: 'Gestion des Stocks', icon: <Package size={18} />, path: '/admin/stock' },
    { id: 'nav-finance', label: 'Finance & Devis', icon: <Calculator size={18} />, path: '/admin/finance' },
    ...
    { id: 'nav-tasks', label: 'Tâches & Projets', icon: <CheckSquare size={18} />, path: '/admin/tasks' },
    { id: 'nav-cms', label: 'Éditeur de site (CMS)', icon: <Globe size={18} />, path: '/admin/cms/hero' },
    { id: 'nav-settings', label: 'Paramètres', icon: <Settings size={18} />, path: '/admin/settings' },
];
            ...
            <div className="fixed top-[15%] left-1/2 -translate-x-1/2 w-full max-w-2xl bg-white rounded-sm shadow-2xl z-[101] overflow-hidden border border-slate-200 animate-in fade-in zoom-in-95 duration-200">
                            ...
                            {filteredItems.map((item, idx) => (
                                ...
                                >
                                    <div className="flex items-center gap-4">
                                        <div className={`p-2 rounded-sm ${idx === selectedIndex ? 'bg-white/20' : 'bg-slate-100'}`}>
                                            {item.icon}
                                        </div>
                                        <span className="font-bold">{item.label}</span>
                                    </div>
                                    <div className={`flex items-center gap-2 transition-transform duration-300 ${idx === selectedIndex ? 'translate-x-0 opacity-100' : 'translate-x-4 opacity-0'}`}>
                ...
                <div className="px-6 py-4 bg-slate-50 border-t border-slate-100 flex justify-between items-center text-[10px] font-black text-slate-400 uppercase tracking-widest text-center">
                    <div className="flex gap-4">
                        <span className="flex items-center gap-1"><kbd className="px-2 py-1 bg-white border border-slate-200 rounded shadow-sm">↵</kbd> Valider</span>
                        <span className="flex items-center gap-1"><kbd className="px-2 py-1 bg-white border border-slate-200 rounded shadow-sm">↑↓</kbd> Naviguer</span>
                    </div>
                    <div>
                        Tapez pour filtrer les résultats
                    </div>
                </div>
            </div>
        </>
