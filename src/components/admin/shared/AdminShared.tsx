...
export const InputField: React.FC<{
  ...
  helper?: string;
  placeholder?: string;
  type?: 'text' | 'textarea' | 'email' | 'number' | 'date' | 'select' | 'password' | 'time';
...
export const Modal: React.FC<{
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4 overflow-y-auto">
      <div
        className={`bg-white rounded-sm shadow-2xl w-full ${sizeClasses[size]} flex flex-col max-h-full animate-in fade-in zoom-in duration-200`}
      >
        {/* Header */}
...
export const Badge: React.FC<{
  ...
  return (
    <span
      className={`px-2 py-0.5 rounded-full text-[10px] font-bold uppercase tracking-wider ${variants[variant][color]} ${className}`}
    >
      {children}
    </span>
  );
...
export const AdminTable: React.FC<{
  ...
  className?: string;
}> = ({ headers, data, renderRow, onRowClick, className = "" }) => (
      ...
      <thead>
        <tr className="border-b border-slate-100 bg-slate-50/50">
          {headers.map((header) => (
            <th
              key={header}
              className="py-4 px-4 text-[10px] font-black text-slate-400 uppercase tracking-widest"
            >
              {header}
            </th>
          ))}
