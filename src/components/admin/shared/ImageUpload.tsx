...
export const ImageUpload: React.FC<ImageUploadProps> = ({
  ...
  return (
    <div className={`space-y-2 ${className}`}>
      ...
      <div
        className={`relative group border-2 border-dashed rounded-sm transition-all h-40 flex flex-col items-center justify-center overflow-hidden bg-slate-50/50 ${value ? 'border-sky-200' : 'border-slate-200 hover:border-sky-300 hover:bg-white'}`}
      >
        {value ? (
          <>
          ...
          >
            ...
            <span className="text-xs font-bold uppercase tracking-wide">
              {uploading ? 'Téléchargement...' : 'Cliquer pour uploader'}
            </span>
          </button>
        )}
