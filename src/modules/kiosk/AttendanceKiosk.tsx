...
import { Camera, ShieldCheck, UserCheck, Clock, AlertCircle, ArrowLeft, RefreshCw, Smartphone, ChevronRight, Zap } from 'lucide-react';
...
export const AttendanceKiosk: React.FC = () => {
    ...
    const [status, setStatus] = useState<'idle' | 'loading_ai' | 'detecting' | 'recognizing' | 'success' | 'error'>('loading_ai');
    ...
    const fetchData = async () => {
        try {
            ...
            const data = await apiFetch(`/erp/${companyId}/kiosk/employees`);
            setEmployees(data);

            const attendances = await apiFetch(`/erp/${companyId}/kiosk/stats`);
    ...
    const handleAutoValidate = async (employee: any) => {
        ...
        try {
            await apiFetch(`/erp/${companyId}/kiosk/attendance`, {
                method: 'POST',
                body: JSON.stringify({
        ...
        <div className="min-h-screen bg-[#0f172a] text-white flex flex-col font-sans overflow-hidden">
            ...
            <div className="relative z-10 p-8 flex justify-between items-center bg-slate-900/50 backdrop-blur-xl border-b border-white/5">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        <h1 className="text-2xl font-black tracking-tighter">KIOSQUE <span className="text-sky-500 text-sm ml-1 font-bold uppercase tracking-widest">ENEA ERP</span></h1>
                        <p className="text-[10px] text-slate-400 font-bold uppercase tracking-[0.2em]">Espace Public de Pointage</p>
                    </div>
                </div>

                <div className="flex items-center gap-10">
                    <div className="text-right">
                        <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest mb-1">Présences Aujourd'hui</p>
                    ...
                    <div className="text-right">
                        <p className="text-3xl font-black tabular-nums">{currentTime.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</p>
                        <p className="text-[10px] text-sky-500 font-bold uppercase tracking-widest">{currentTime.toLocaleDateString('fr-FR', { weekday: 'long', day: 'numeric', month: 'long' })}</p>
                    </div>
                </div>
            </div>
                ...
                <div className="flex-[1.5] flex flex-col gap-6">
                    <div className="relative flex-1 bg-black rounded-3xl overflow-hidden border-4 border-white/5 shadow-2xl group">
                        ...
                        <div className="absolute bottom-6 left-1/2 -translate-x-1/2 flex items-center gap-4 bg-slate-900/80 backdrop-blur-xl px-4 py-2 rounded-full border border-white/10 z-20">
                            <div className={`w-2 h-2 rounded-full ${faceDetected ? 'bg-green-500 shadow-[0_0_10px_#22c55e]' : 'bg-red-500'} animate-pulse`}></div>
                            <p className="text-[10px] font-black uppercase tracking-widest text-slate-400">
                                {faceDetected ? (selectedEmployee ? 'Sujet Identifié' : 'Visage détecté (Inconnu)') : 'Recherche de Sujet...'}
                            </p>
                            {faceDetected && (
                                <div className="flex items-center gap-2 border-l border-white/10 pl-4">
                                    <div className="w-20 h-1.5 bg-white/10 rounded-full overflow-hidden">
                                        <div className="h-full bg-sky-500 transition-all duration-300" style={{ width: `${aiConfidence}%` }}></div>
                                    </div>
                        ...
                        <div className="absolute inset-0 pointer-events-none">
                            {/* Scanning frame */}
                            <div className={`absolute inset-[15%] border-2 rounded-[40px] transition-all duration-500 ${faceDetected ? 'border-sky-500/50 border-solid scale-105' : 'border-white/10 border-dashed'}`}>
                                {status === 'idle' && !faceDetected && (
                                    <div className="absolute inset-0 flex items-center justify-center">
                                        <div className="bg-sky-500/10 border border-sky-500/20 backdrop-blur-md px-6 py-4 rounded-2xl text-center animate-pulse">
                                            <p className="text-sky-400 font-black uppercase tracking-[0.2em] text-xs">Approchez-vous de la caméra</p>
                                        </div>
                                    </div>
                                )}
                            </div>
                            ...
                            {/* Corner Accents */}
                            <div className={`absolute top-10 left-10 w-20 h-20 border-t-4 border-l-4 rounded-tl-3xl transition-all duration-500 ${status === 'success' ? 'border-green-500' : faceDetected ? 'border-sky-500 scale-110' : 'border-slate-700'}`}></div>
                            <div className={`absolute top-10 right-10 w-20 h-20 border-t-4 border-r-4 rounded-tr-3xl transition-all duration-500 ${status === 'success' ? 'border-green-500' : faceDetected ? 'border-sky-500 scale-110' : 'border-slate-700'}`}></div>
                            <div className={`absolute bottom-10 left-10 w-20 h-20 border-b-4 border-l-4 rounded-bl-3xl transition-all duration-500 ${status === 'success' ? 'border-green-500' : faceDetected ? 'border-sky-500 scale-110' : 'border-slate-700'}`}></div>
                            <div className={`absolute bottom-10 right-10 w-20 h-20 border-b-4 border-r-4 rounded-br-3xl transition-all duration-500 ${status === 'success' ? 'border-green-500' : faceDetected ? 'border-sky-500 scale-110' : 'border-slate-700'}`}></div>

                            {/* Scan Line */}
                            {(status === 'detecting' || status === 'recognizing' || (status === 'idle' && faceDetected)) && (
                                    ...
                                    {status === 'loading_ai' ? (
                                        <div className="flex flex-col items-center gap-6">
                                            ...
                                            <div className="space-y-2">
                                                ...
                                                <p className="text-sky-400 font-bold uppercase tracking-widest text-[10px] animate-pulse">Initialisation des modèles neuronaux...</p>
                                            </div>
                                        </div>
                                        ...
                                        <div className="flex flex-col items-center gap-6">
                                            ...
                                            <div className="space-y-2">
                                                <p className="text-2xl font-black text-white">{status === 'detecting' ? 'DÉTECTION...' : 'IDENTIFICATION...'}</p>
                                                <p className="text-sky-400 font-bold uppercase tracking-widest text-xs animate-pulse">{message}</p>
                                            </div>
                                        </div>
                                    ) : status === 'success' ? (
                                        <div className="flex flex-col items-center gap-6">
                                            ...
                                            <div className="space-y-2">
                                                ...
                                                <p className="text-green-400 font-bold text-xl">{selectedEmployee?.fullName}</p>
                    ...
                    <div className="bg-slate-900/50 backdrop-blur-xl rounded-2xl border border-white/5 p-6 flex items-center justify-between">
                        <div className="flex items-center gap-4 text-slate-400">
                            ...
                            <p className="text-sm font-medium">Système sécurisé par reconnaissance faciale biométrique</p>
                        </div>
                        <button
                ...
                <div className="flex-1 flex flex-col gap-6 overflow-hidden">
                    ...
                    <div className="flex-1 bg-slate-900/30 rounded-3xl border border-white/5 overflow-hidden flex flex-col p-4">
                        <div className="mb-6 px-2 flex justify-between items-center">
                            <h2 className="text-sm font-black uppercase tracking-widest text-slate-400">Collaborateurs</h2>
                            <button onClick={fetchData} className="p-2 hover:bg-white/5 rounded-lg active:rotate-180 transition-transform duration-500">
                        ...
                        <div className="flex-1 overflow-y-auto space-y-2 pr-2 custom-scrollbar">
                            {employees.map((emp) => (
                                <button
                                    ...
                                    className={`w-full group relative flex items-center gap-4 p-4 rounded-2xl transition-all duration-300 border ${selectedEmployee?.id === emp.id
                                        ? 'bg-sky-500/10 border-sky-500/50 shadow-lg shadow-sky-500/5'
                                        : 'bg-white/5 border-transparent hover:bg-white/10 hover:border-white/10'}`}
                                >
                                    <div className={`w-12 h-12 rounded-xl flex items-center justify-center font-black text-sm transition-all duration-300 ${selectedEmployee?.id === emp.id ? 'bg-sky-500 text-white shadow-lg shadow-sky-500/20' : 'bg-slate-800 text-slate-400'}`}>
                                        {emp.fullName.split(' ').map((n: string) => n[0]).join('')}
                                    </div>
                                    <div className="text-left flex-1">
                                        <p className={`font-bold transition-colors ${selectedEmployee?.id === emp.id ? 'text-sky-400' : 'text-slate-200'}`}>{emp.fullName}</p>
                                        <p className="text-[10px] text-slate-500 font-bold uppercase tracking-widest">{emp.position}</p>
                                    </div>
                                    <div className={`transition-all duration-300 ${selectedEmployee?.id === emp.id ? 'translate-x-0 opacity-100' : '-translate-x-2 opacity-0'}`}>
                        ...
                        <div className="mt-6 pt-6 border-t border-white/5">
                            <button
                                ...
                                className={`w-full py-5 rounded-2xl font-black uppercase tracking-[0.2em] transition-all flex items-center justify-center gap-3 overflow-hidden group relative ${!selectedEmployee || status !== 'idle'
                                    ? 'bg-slate-800 text-slate-600 grayscale'
                                    : 'bg-sky-600 text-white hover:bg-sky-500 shadow-2xl shadow-sky-600/30 active:scale-95'}`}
                            >
                                <Camera size={20} />
                                {status === 'idle'
                                    ? (selectedEmployee ? `CONFIRMER ${selectedEmployee.fullName.split(' ')[0]}` : 'Valider Présence')
                                    : 'En cours...'}
