...
export const PrintingSettings: React.FC<PrintingSettingsProps> = ({ companyId }) => {
    const { fetchPrinterSettings, savePrinterSettings: savePrinterSettingsApi, discoverPrinters } = useSettings(companyId);
    ...
    useEffect(() => {
        fetchPrinterSettings().then((data: any) => {
            if (data) {
                setPrinterSettings({
    ...
    return (
        <div className="space-y-6">
            <Card title="Configuration des Imprimantes" subtitle="Gérez les imprimantes serveur et les formats d'étiquettes">
                <div className="flex justify-between items-center mb-6">
                    <p className="text-sm text-slate-500">
                        Configurez les imprimantes connectées au serveur pour l'impression automatique des factures et étiquettes.
                    </p>
                ...
                {discoveredPrinters.length > 0 && (
                    <div className="mb-6 bg-emerald-50 border border-emerald-100 p-4 rounded-sm animate-in fade-in slide-in-from-top-2">
                        <h4 className="text-sm font-bold text-emerald-800 mb-2">Imprimantes détectées sur le réseau :</h4>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
                            {discoveredPrinters.map((p: any) => (
                                <button
                                    key={p.name}
                                    onClick={() => {
                                        if (confirm(`Utiliser "${p.name}" comme imprimante par défaut pour les documents ?\n(Annuler pour définir comme imprimante étiquettes)`)) {
                                ...
                                >
                                    <div className="overflow-hidden">
                                        <div className="flex items-center gap-2 mb-0.5">
                                            ...
                                            <span className={`text-[9px] px-1 rounded-full font-bold uppercase ${p.type === 'INSTALLED' ? 'bg-slate-200 text-slate-600' : 'bg-blue-100 text-blue-600'}`}>
                                                {p.type === 'INSTALLED' ? 'Local' : 'Réseau'}
                                            </span>
                                        </div>
                                        <div className="text-[10px] text-slate-500 truncate">{p.details || 'Prêt'}</div>
                                    </div>
                                    <span className="opacity-0 group-hover:opacity-100 text-[10px] bg-emerald-200 text-emerald-800 font-bold px-1.5 py-0.5 rounded whitespace-nowrap">Choisir</span>
                                </button>
                            ))}
                        </div>
                    </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                    <div className="space-y-4">
                        <div className="flex items-center gap-2 border-b border-slate-100 pb-2">
                            ...
                            <h4 className="text-sm font-black text-slate-800 uppercase tracking-widest">Documents Standards</h4>
                        </div>
                        <div className="space-y-4">
                            <InputField
                            ...
                            />
                            <p className="text-xs text-slate-400 italic">Si laissé vide, l'imprimante par défaut du système hôte sera utilisée.</p>
                        </div>
                    </div>
                    ...
                    <div className="space-y-4">
                        <div className="flex items-center gap-2 border-b border-slate-100 pb-2">
                            ...
                            <h4 className="text-sm font-black text-slate-800 uppercase tracking-widest">Code-Barres & Étiquettes</h4>
                        </div>
                        <div className="space-y-4">
                            <InputField
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                <InputField
                                    ...
                                    onChange={(v) => setPrinterSettings({ ...printerSettings, labelWidth: Number(v) })}
                                />
                                <InputField
                                    type="number"
                                    label="Hauteur (mm)"
                                    value={printerSettings.labelHeight}
                                    onChange={(v) => setPrinterSettings({ ...printerSettings, labelHeight: Number(v) })}
                                />
                            </div>
                        </div>
                    </div>
