    ...
    >
      <div className="space-y-6">
        <InputField
          ...
          value={deal.title}
          onChange={(v) => onChange({ ...deal, title: v })}
          placeholder="Ex: Contrat Fibre Plateau, Maintenance Annuelle..."
        />
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="flex gap-2 items-end">
            <div className="flex-1">
              <InputField
                ...
                type="number"
                value={deal.amount}
                onChange={(v) => onChange({ ...deal, amount: Number(v) })}
              />
            </div>
            <div className="w-32 space-y-1.5">
              ...
              <select
                className="w-full px-3 py-2.5 rounded-sm border border-slate-200 bg-white text-sm outline-none focus:ring-4 focus:ring-sky-500/10 focus:border-sky-500 transition-all font-bold"
                value={deal.currency || 'XOF'}
                onChange={(e) => {
                  const curr = currencies.find(c => c.code === e.target.value);
                  onChange({
                    ...deal,
                    currency: e.target.value,
                    currencyRate: curr?.rate || 1.0
                  });
                }}
              >
                {currencies.map((c) => (
          ...
          <InputField
            ...
            onChange={(v) => onChange({ ...deal, probability: Number(v) })}
          />
        </div>

        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <InputField
            label="Étape de vente"
            type="select"
            value={deal.stage}
            onChange={(v) => onChange({ ...deal, stage: v as any })}
            options={[
          ...
          <InputField
            ...
            type="date"
            value={deal.closingDate ? new Date(deal.closingDate).toISOString().split('T')[0] : ''}
            onChange={(v) => onChange({ ...deal, closingDate: v })}
          />
        </div>
      </div>
    </Modal>
