      ...
      {activeTab === 'contacts' ? (
        <>
          <SectionHeader
            ...
            subtitle="Gérez vos relations clients et prospects."
            actions={
              <button
                onClick={() =>
                  setEditingContact({
          ...
          <DataTable<CRMContact>
            ...
            columns={[
              ...
              },
              {
                ...
                accessor: (c) => (
                  <Badge
                    color={
                      c.type === 'Client' ? 'blue' : c.type === 'Prospect' ? 'purple' : 'slate'
                    }
                  >
                    {c.type}
                  </Badge>
              ...
              },
              {
                ...
                accessor: (c) => (
                  <Badge
                    color={c.status === 'Actif' ? 'green' : c.status === 'Lead' ? 'amber' : 'slate'}
                  >
                    {c.status}
                  </Badge>
                ),
            ...
            actions={(c) => (
              <div className="flex justify-end gap-1 opacity-10 md:opacity-0 group-hover:opacity-100 transition-opacity">
                ...
                <button
                  onClick={() => {
                    if (confirm('Supprimer ce contact ?')) deleteContact(c.id);
                  }}
        ...
        <>
          <SectionHeader
            ...
            subtitle="Suivez vos opportunités en cours et vos ventes."
            actions={
              <button
                onClick={() =>
                  setEditingDeal({
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
            {['Nouveau', 'Proposition', 'Négociation', 'Gagné'].map((stage) => {
              ...
              return (
                <div
                  key={stage}
                  className="bg-white p-5 rounded-sm border border-slate-200 shadow-sm"
                >
                  ...
                  <div className="text-xl font-black text-slate-900">
                    ...
                    <span className="text-[10px] font-normal text-slate-400">
                      {currencies.find((c: any) => c.isBase)?.code || 'XOF'}
                    </span>
                  </div>
                </div>
              );
          ...
          <DataTable<CRMDeal>
              ...
              },
              {
                ...
                accessor: (d) => (
                  <span className="text-slate-500 font-medium">
                    {contacts.find((c) => c.id === d.contactId)?.companyName ||
                      contacts.find((c) => c.id === d.contactId)?.contactName ||
                      'Client Inconnu'}
                  </span>
                ),
                sortable: false,
                ...
                accessor: (d) => (
                  <div className="py-1">
                    <div className="font-black text-slate-900">
                      {d.amount.toLocaleString()} {d.currency || 'XOF'}
                    </div>
              ...
              },
              {
                ...
                accessor: (d) => (
                  <div className="inline-flex items-center gap-3">
                    <Badge
                      color={d.probability > 70 ? 'green' : d.probability > 30 ? 'blue' : 'slate'}
                    >
                      {d.stage}
                    </Badge>
            ...
            actions={(d) => (
              <div className="flex justify-end gap-1 opacity-10 md:opacity-0 group-hover:opacity-100 transition-opacity">
                ...
                <button
                  onClick={() => {
                    if (confirm('Supprimer cette opportunité ?')) deleteDeal(d.id);
                  }}
                  className="p-2 text-slate-400 hover:text-red-600 hover:bg-red-50 rounded-sm transition-all"
                >
                  <Trash2 size={18} />
