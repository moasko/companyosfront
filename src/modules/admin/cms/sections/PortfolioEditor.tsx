        ...
        <div className="space-y-8">
            <div className="bg-white p-8 rounded-sm border shadow-sm space-y-6">
                ...
                <div className="space-y-4">
                    <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest block ml-1">Projets / Réalisations (Liste de textes)</label>
                    <textarea
                        ...
                        onChange={e => onChange({ ...data, works: e.target.value.split('\n').filter(w => w) })}
                ...
                <div className="space-y-4 pt-4 border-t border-slate-100">
                    <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest block ml-1">Ils nous font confiance (Clients / Partenaires)</label>
                    <textarea
                        className="w-full px-4 py-3 rounded-sm border border-slate-200 text-sm font-semibold rows-3 focus:ring-1 focus:ring-sky-500 outline-none"
                        value={(data.clients || []).join('\n')}
                        onChange={e => onChange({ ...data, clients: e.target.value.split('\n').filter(w => w) })}
                        placeholder="ORANGE&#10;MOOV&#10;SOCIETE GENERALE"
                        rows={4}
                    />
            ...
            <div className="space-y-4">
                <div className="flex justify-between items-center">
                    ...
                    <button
                        onClick={() => onChange({ ...data, gallery: [...data.gallery, { caption: '', category: 'General', imageUrl: '' }] })}
                ...
                <div className="grid grid-cols-2 lg:grid-cols-3 gap-6">
                    {data.gallery.map((img, idx) => (
                        <div key={idx} className="bg-white rounded-sm border shadow-sm overflow-hidden group p-4 space-y-4">
                            ...
                            <div className="grid grid-cols-2 gap-3">
                                <InputField label="Catégorie" value={img.category} onChange={v => {
                                    const newGallery = [...data.gallery];
                                    newGallery[idx].category = v;
                                    onChange({ ...data, gallery: newGallery });
                                }} placeholder="ex: Maintenance" />
                                <InputField label="Légende" value={img.caption} onChange={v => {
                                    const newGallery = [...data.gallery];
