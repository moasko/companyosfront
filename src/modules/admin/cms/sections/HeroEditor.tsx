    ...
    return (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
            ...
            <div className="space-y-4">
                <h4 className="font-bold text-slate-500 text-xs uppercase tracking-wider">Aperçu Simplifié</h4>
                <div className="bg-slate-900 rounded-sm p-0 text-white flex flex-col justify-center relative overflow-hidden h-[400px]">
                    {data.imageUrl && (
                        <img src={data.imageUrl} alt="" className="absolute inset-0 w-full h-full object-cover opacity-40" />
                    )}
                    <div className="relative z-10 p-8 space-y-4 max-w-md">
                        <Badge color="blue">{data.tagline || 'Slogan'}</Badge>
                        <h1 className="text-4xl font-black leading-tight">{data.title || 'Titre de votre site'}</h1>
                        <p className="text-slate-400 text-sm leading-relaxed">{data.subtitle || 'Votre texte de présentation apparaîtra ici pour vos visiteurs.'}</p>
                        <div className="pt-4">
                            <button className="bg-sky-600 px-6 py-2 rounded-sm font-bold text-sm">Découvrir nos services</button>
                        </div>
                    </div>
                </div>
            </div>
