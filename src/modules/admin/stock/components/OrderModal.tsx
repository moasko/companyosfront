        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 bg-slate-50 p-6 rounded-sm border border-slate-100">
          <div className="space-y-1">
            <InputField
              ...
              onChange={(v) => onChange({ ...order, reference: v })}
          ...
          <div className="space-y-1">
            <InputField
              ...
              label="Statut"
              value={order.status}
              onChange={(v) => onChange({ ...order, status: v as any })}
              options={[
          ...
          <InputField
            ...
            onChange={(v) => onChange({ ...order, date: v })}
          />
          <InputField
            type="date"
            label="Date prévue de livraison"
            value={
              order.expectedDate ? new Date(order.expectedDate).toISOString().split('T')[0] : ''
            }
            onChange={(v) => onChange({ ...order, expectedDate: v })}
          />
        </div>
                    ...
                    <td colSpan={6} className="p-12 text-center">
                      <div className="flex flex-col items-center gap-2 opacity-30">
                        ...
                        <p className="text-sm italic font-medium">
                          Ajoutez des articles à commander.
                        </p>
                      </div>
                    </td>
                  </tr>
