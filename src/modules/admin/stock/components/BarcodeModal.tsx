...
export const BarcodeModal: React.FC<Props> = ({ isOpen, onClose, product }) => {
    ...
    return (
        <Modal
            isOpen={isOpen}
            onClose={onClose}
            title="Générateur d'Étiquette Code-Barres"
            size="md"
        >
            <div className="space-y-8 p-6">
                <div className="flex flex-col items-center justify-center p-8 bg-slate-50 border border-slate-200 border-dashed rounded-sm">
                    <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-4">Aperçu de l'étiquette (50x30mm)</p>
                    ...
                    <div className="bg-white shadow-xl border border-slate-200 p-4 w-64 h-40 flex flex-col items-center justify-center text-center relative overflow-hidden">
                        ...
                        <p className="text-[8px] font-bold text-slate-400 uppercase mb-1">{product.brand || 'ENEA TELECOM'}</p>
                        ...
                        <p className="font-mono text-[9px] text-slate-500 mt-1">{product.barcode || product.ref}</p>

                        {product.sellingPrice && (
                            <p className="text-sm font-black text-sky-600 mt-2">{product.sellingPrice.toLocaleString()} CFA</p>
                        )}
                    </div>
                </div>
                ...
                <div className="flex flex-col gap-3">
                    <div className="bg-sky-50 p-4 rounded-sm border border-sky-100 mb-2">
                        <div className="flex gap-3">
                            <div className="bg-white p-2 rounded-sm text-sky-600 border border-sky-200 shadow-sm"><ScanBarcode size={24} /></div>
                            <div>
                                <p className="text-sm font-bold text-sky-900">Format Standard (Zèbre/Dymo)</p>
                                <p className="text-xs text-sky-700/70">Prêt pour impression sur imprimante thermique d'étiquettes.</p>
                            </div>
                        </div>
                    </div>
