            ...
            footer={
                <div className="flex gap-2 w-full justify-between items-center">
                    <div className="text-sm text-slate-500 font-medium">
                        {movement.status === 'Validé' ? '🔒 Ce mouvement est validé et ne peut plus être modifié.' : '📝 Brouillon'}
                    </div>
                    <div className="flex gap-3">
                        <button onClick={onClose} className="px-6 py-2.5 text-slate-600 font-semibold hover:bg-slate-100 rounded-sm transition-all">Fermer</button>
                        {movement.status !== 'Validé' && (
                            <>
                                <button onClick={onSave} className="px-6 py-2.5 bg-slate-200 text-slate-800 font-bold rounded-sm hover:bg-slate-300 transition-all">Sauvegarder Brouillon</button>
                                <button onClick={onValidate} className="px-8 py-2.5 bg-green-600 text-white font-bold rounded-sm hover:bg-green-700 shadow-lg shadow-green-900/10 transition-all">Valider Stock</button>
                            </>
                        )}
                    </div>
                </div>
            ...
            <div className="space-y-6">
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6 p-6 bg-slate-50 rounded-sm border border-slate-100">
                    ...
                    <div className="col-span-3">
                        <label className="block text-[10px] font-black text-slate-500 uppercase tracking-widest mb-1.5 ml-1">
                            {movement.type === 'Reception' ? 'Fournisseur d\'origine' : 'Client / Projet de destination'}
                        </label>
                        <select
                        ...
                        >
                            ...
                            {movement.type === 'Reception'
                                ...
                                : crmContacts.map(c => <option key={c.id} value={c.id}>{c.companyName}</option>)
                            }
                        </select>
                    </div>
                </div>
                ...
                <div>
                    <div className="flex justify-between items-center mb-4">
                        ...
                        {movement.status !== 'Validé' && (
                            <button onClick={handleAddItem} className="text-xs bg-sky-50 text-sky-700 px-4 py-2 rounded-sm font-bold hover:bg-sky-100 transition-all border border-sky-100 flex items-center gap-2">
                                    ...
                                    <tr key={idx} className="group hover:bg-slate-50/50">
                                        <td className="p-3">
                                            ...
                                            >
                                                ...
                                                {stock.map(p => <option key={p.id} value={p.id}>{p.name} ({p.quantity} {p.unit} dispo)</option>)}
                                            </select>
                                            <div className="text-[10px] text-slate-400 mt-1 pl-1 font-bold italic">{item.description}</div>
                                        </td>
                                        <td className="p-3">
                                            <input
                                                type="number"
                    ...
                    <div className="flex justify-end mt-6">
                        <div className="bg-slate-900 text-white p-6 rounded-sm w-full md:w-80 shadow-2xl relative overflow-hidden group">
                            ...
                            <div className="relative z-10">
                                <div className="text-[10px] font-black opacity-50 uppercase tracking-widest mb-1">VALEUR TOTALE DU TRANSIT</div>
                                <div className="text-3xl font-black">
                                    {totalValue.toLocaleString()} <span className="text-sm font-normal opacity-40">CFA</span>
                                </div>
                            </div>
                        </div>
                    </div>
