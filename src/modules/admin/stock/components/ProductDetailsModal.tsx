...
export const ProductDetailsModal: React.FC<ProductDetailsModalProps> = ({
  ...
  const productMovements = movements
    .filter((m) => m.items.some((item) => item.stockId === product.id))
    ...
    <Modal
      ...
      footer={
        <div className="flex justify-between items-center w-full">
          <button
            onClick={onClose}
            className="px-6 py-2 text-slate-500 font-bold hover:bg-slate-100 rounded-sm transition-all"
          >
            Fermer
          </button>
          <button
          ...
          <div className="w-full md:w-64 shrink-0">
            {product.imageUrl ? (
              <img
                ...
                alt={product.name}
                className="w-full aspect-square object-cover rounded-sm border border-slate-100 shadow-xl"
              />
            ) : (
            ...
            <div className="mt-6 space-y-3">
              <div className="bg-slate-50 p-4 rounded-sm border border-slate-100">
                ...
                <div className="flex items-end justify-between">
                  <p
                    className={`text-3xl font-black ${isLowStock ? 'text-red-600' : 'text-slate-900'}`}
                  >
                    {product.quantity}{' '}
                    <span className="text-sm font-bold opacity-40">{product.unit}</span>
                  </p>
                  <Badge color={isLowStock ? 'red' : 'green'}>
                    {isLowStock ? 'Alerte' : 'Optimal'}
                  </Badge>
                </div>
                {isLowStock && (
          ...
          <div className="flex-1 space-y-6 w-full">
            <div className="space-y-1">
              <div className="flex items-center gap-2 mb-2">
                <Badge color="blue">
                  {product.categoryRel?.name || product.category || 'Non classé'}
                </Badge>
                <Badge color="slate">{product.type}</Badge>
              </div>
              <h2 className="text-3xl font-black text-slate-900 tracking-tight">{product.name}</h2>
              <p className="font-mono text-sm text-slate-400 font-bold tracking-tighter uppercase">
                {product.ref} • {product.barcode || 'Pas de code-barres'}
              </p>
            </div>
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div className="p-4 bg-slate-50/50 rounded-sm border border-slate-100 flex flex-col items-center justify-center overflow-hidden">
                <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1 w-full text-left">
                  Code-Barres
                </p>
                {product.barcode ? (
              ...
              <div
                className={`p-3 rounded-sm border flex flex-col justify-center ${product.expiryDate && new Date(product.expiryDate) < new Date(Date.now() + 30 * 24 * 60 * 60 * 1000) ? 'bg-red-50 border-red-100' : 'bg-slate-50/50 border-slate-100'}`}
              >
                <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-0.5">
                  Expiration
                </p>
                <p
                  className={`font-black text-xs ${product.expiryDate && new Date(product.expiryDate) < new Date(Date.now() + 30 * 24 * 60 * 60 * 1000) ? 'text-red-600' : 'text-slate-800'}`}
                >
                ...
                {productMovements.map((m) => {
                  ...
                  return (
                    <tr key={m.id} className="hover:bg-slate-50/50 transition-all group">
                      ...
                      <td className="px-6 py-4">
                        <div
                          className={`flex items-center gap-2 text-xs font-black uppercase ${isEntry ? 'text-green-600' : 'text-amber-600'}`}
                        >
                      ...
                      <td className="px-6 py-4 text-center">
                        <span
                          className={`text-sm font-black ${isEntry ? 'text-green-700' : 'text-slate-800'}`}
                        >
                          {isEntry ? '+' : '-'}
                  ...
                  <tr>
                    <td colSpan={5} className="py-20 text-center">
                      ...
                      <p className="text-slate-400 font-medium text-sm">
                        Aucun mouvement historique pour cet article.
                      </p>
                    </td>
                  </tr>
                )}
