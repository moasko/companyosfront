    ...
    <Modal
      ...
      title={
        product.id && !product.id.startsWith('new-')
          ? "Éditer l'article"
          : "Nouvel article dans l'inventaire"
      }
      size="lg"
      footer={
        <div className="flex gap-3 justify-end w-full">
          <button
            onClick={onClose}
            className="px-6 py-2 text-slate-600 font-semibold hover:bg-slate-100 rounded-sm transition-all"
          >
            Annuler
          </button>
          <button
            onClick={onSave}
            className="px-8 py-2 bg-sky-600 text-white font-bold rounded-sm hover:bg-sky-700 shadow-lg shadow-sky-900/10 transition-all flex items-center gap-2"
          >
      ...
      <div className="space-y-6">
          ...
          <div className="flex-1 space-y-4">
              ...
              <div className="space-y-1">
                ...
                <div className="flex gap-2">
                  ...
                  <button
                    ...
                    className="p-2 bg-slate-100 rounded-sm hover:bg-slate-200 text-slate-600 border border-slate-200"
                  >
                    <Wand2 size={18} />
                  </button>
                </div>
            ...
            <div className="grid grid-cols-2 gap-4">
              <InputField
                ...
                value={product.brand || ''}
                onChange={(v) => onChange({ ...product, brand: v })}
                placeholder="Ex: Cisco, Legrand..."
              />
              <div className="space-y-1">
                <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">
                  Catégorie
                ...
                <div className="flex gap-2">
                  ...
                  <button
                    ...
                    className="p-2 bg-slate-100 rounded-sm hover:bg-slate-200 text-slate-600"
                  >
                    <Plus size={18} />
                  </button>
                </div>
        ...
        <div className="grid grid-cols-2 gap-6">
          ...
          <InputField
            ...
            onChange={(v) => onChange({ ...product, location: v })}
        ...
        <div className="grid grid-cols-3 gap-6 pt-4 border-t border-slate-100">
          <InputField
            ...
            onChange={(v) => onChange({ ...product, batchNumber: v })}
          ...
          <InputField
            ...
            onChange={(v) => onChange({ ...product, manufacturingDate: v })}
          />
          <InputField
            type="date"
            label="Date d'Expiration"
            value={product.expiryDate || ''}
            onChange={(v) => onChange({ ...product, expiryDate: v })}
          />
        </div>
        ...
        <div className="grid grid-cols-4 gap-4 bg-slate-50 p-6 rounded-sm border border-slate-100">
          <InputField
            ...
            value={product.type}
            onChange={(v) => onChange({ ...product, type: v as any })}
            options={[
              { value: 'Produit', label: 'Produit Stocké' },
              { value: 'Service', label: 'Service / Travaux' },
            ]}
          />
          <InputField
            type="number"
            ...
            onChange={(v) => onChange({ ...product, quantity: Number(v) })}
            disabled={product.type === 'Service'}
          />
          <InputField
            label="Unité"
            value={product.unit}
            onChange={(v) => onChange({ ...product, unit: v })}
            placeholder="u, m, kg..."
          />
          <InputField
            type="number"
            ...
            onChange={(v) => onChange({ ...product, minThreshold: Number(v) })}
          />
        </div>
        ...
        <div className="grid grid-cols-2 gap-6">
          <InputField
            type="number"
            label="Prix de revient (CFA)"
            value={product.value}
            onChange={(v) => onChange({ ...product, value: Number(v) })}
          />
          <InputField
            type="number"
            label="Prix de vente (CFA)"
            value={product.sellingPrice || 0}
            onChange={(v) => onChange({ ...product, sellingPrice: Number(v) })}
          />
        </div>
