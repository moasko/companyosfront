...
export const HRModule: React.FC<HRModuleProps> = ({ companyId, allCompanies = [] }) => {
  ...
  const {
    ...
    createLeaveRequest,
    updateLeaveRequest,
  } = useHr(companyId, { year: selectedYear, month: selectedMonth });
  const [activeTab, setActiveTab] = useState<'employees' | 'payroll' | 'attendance' | 'leaves'>(
    'employees',
  );
  ...
  const handleGeneratePayslip = () => {
    ...
    const primes =
      (generatingPayslip.transportPrime || 0) +
      (generatingPayslip.housingPrime || 0) +
      (generatingPayslip.otherBonuses || 0);
    const gross = base + primes;
    const deductions =
      (generatingPayslip.cnpsDeduction || 0) +
      (generatingPayslip.taxDeduction || 0) +
      (generatingPayslip.otherDeductions || 0);
    const net = gross - deductions;

    // Calcul Part Patronale (Portail Employeur / Partail Emplayer)
    const cnpsEmp = generatingPayslip.cnpsEmployer || gross * 0.077;
    const taxEmp = generatingPayslip.taxEmployer || gross * 0.015;

    createPayslip({
      ...generatingPayslip,
      grossSalary: gross,
      netSalary: net,
      cnpsEmployer: cnpsEmp,
      taxEmployer: taxEmp,
      totalEmployer: cnpsEmp + taxEmp,
    ...
    <div className="space-y-8 animate-in fade-in duration-500">
      ...
      <div className="bg-white p-6 rounded-sm border border-slate-200 shadow-sm">
        <div className="space-y-1">
          <h1 className="text-2xl font-black text-slate-900 tracking-tight">Ressources Humaines</h1>
          <p className="text-slate-500 text-sm font-medium">
            Gérez votre capital humain et la conformité sociale.
          </p>
        </div>
      </div>
            ...
            columns={[
                ...
                accessor: (emp) => (
                  <div className="flex items-center gap-4 py-1">
                    ...
                    <div>
                      ...
                      <div className="text-xs text-slate-400 font-mono tracking-tighter uppercase">
                        {emp.matricule}
                      </div>
                    </div>
                  </div>
              ...
              },
              {
                ...
                accessor: (emp) => (
                  <Badge
                    color={
                      emp.status === 'Actif' ? 'green' : emp.status === 'Congé' ? 'blue' : 'slate'
                    }
                  >
                    {emp.status}
                  </Badge>
            ...
            actions={(emp) => (
              <div className="flex items-center justify-end gap-1 opacity-10 md:opacity-0 group-hover:opacity-100 transition-all">
                ...
                <button
                  ...
                  className="p-2.5 text-slate-400 hover:text-sky-600 hover:bg-sky-50 rounded-sm transition-all"
                >
                  <Settings size={18} />
                </button>
                <button
                  onClick={() => {
                    if (confirm('Confirmer la suppression ?')) deleteEmployee(emp.id);
                  }}
                  className="p-2.5 text-slate-400 hover:text-red-600 hover:bg-red-50 rounded-sm transition-all"
                >
                  <Trash2 size={18} />
                </button>
                <PDFDownloadLink
                  document={<ContractPDF employee={emp} />}
                  fileName={`Contrat_${emp.fullName.replace(/\s+/g, '_')}.pdf`}
                  className="p-2.5 text-orange-600 hover:bg-orange-50 rounded-sm transition-all"
        ...
        <div className="space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div className="bg-white p-6 rounded-sm border border-slate-200 shadow-sm flex items-center gap-5">
              <div className="p-4 bg-sky-50 text-sky-600 rounded-sm">
                <Wallet size={24} />
              </div>
              <div>
                <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">
            ...
            <div className="bg-white p-6 rounded-sm border border-slate-200 shadow-sm flex items-center gap-5">
              <div className="p-4 bg-purple-50 text-purple-600 rounded-sm">
                <FileText size={24} />
              </div>
              <div>
                <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">
          ...
          <DataTable<Payslip>
            ...
            columns={[
              ...
              },
              {
                header: 'Salaire Net',
                accessor: (ps) => (
                  <div className="font-black text-sky-700 py-1">
                ...
                {ps.status === 'Payé' && (
                  <PDFDownloadLink
                    document={
                      <PayslipPDF
                        payslip={ps}
                        employee={employees.find((e) => e.id === ps.employeeId) || ({} as Employee)}
                      />
                    }
            ...
            columns={[
              ...
              },
              {
                ...
                accessor: (a) => (
                  <Badge
                    color={
                      a.status === 'Présent' ? 'green' : a.status === 'Retard' ? 'amber' : 'red'
                    }
                  >
                    {a.status}
                  </Badge>
          ...
          <DataTable<any>
            ...
            columns={[
                ...
                accessor: (r) => (
                  <div className="py-1">
                    ...
                    <div className="text-[10px] text-slate-400">
                      Du {new Date(r.startDate).toLocaleDateString()} au{' '}
              ...
              },
              {
                header: 'Motif',
                accessor: (r) => (
                  <div className="max-w-xs truncate text-[10px] text-slate-500 italic">
                    {r.reason}
                  </div>
                ),
              },
            ...
            actions={(r) => (
              <div className="flex justify-end gap-1 opacity-10 md:opacity-0 group-hover:opacity-100 transition-all">
                {r.status === 'Pending' && (
                  <>
                    <button
                      onClick={() => updateLeaveRequest({ id: r.id, data: { status: 'Approved' } })}
                      className="p-2 text-green-600 hover:bg-green-50 rounded-sm"
                      title="Approuver"
                    >
                      <ShieldCheck size={18} />
                    </button>
                    <button
                      onClick={() => updateLeaveRequest({ id: r.id, data: { status: 'Rejected' } })}
      ...
      <Modal
        ...
        title={
          editingEmployee?.id?.toString().startsWith('new')
        ...
        footer={
          <div className="flex gap-3 justify-end w-full">
            <button
              ...
              className="px-6 py-2.5 text-slate-500 font-bold hover:bg-slate-100 rounded-sm transition-all"
            >
              Annuler
            </button>
            <button
              onClick={handleSaveEmployee}
              className="px-8 py-2.5 bg-slate-900 text-white font-black rounded-sm hover:bg-black shadow-lg flex items-center gap-2 transition-all"
            >
          ...
          <div className="space-y-8 py-4">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="space-y-4">
                <div className="flex items-center gap-2 text-sky-600 mb-2 font-bold text-xs uppercase tracking-widest">
                  <Users size={14} /> État Civil
                </div>
                <InputField
                  label="Nom Complet"
                  value={editingEmployee.fullName || ''}
                  onChange={(v) => setEditingEmployee({ ...editingEmployee, fullName: v })}
                  icon={<Users size={16} />}
                />
                <div className="grid grid-cols-2 gap-4">
                  <InputField
                    ...
                    onChange={(v) => setEditingEmployee({ ...editingEmployee, matricule: v })}
                  />
                  <InputField
                    label="Date d'entrée"
                    type="date"
              ...
              <div className="space-y-4">
                <div className="flex items-center gap-2 text-sky-600 mb-2 font-bold text-xs uppercase tracking-widest">
                  <Settings size={14} /> Poste & Contrat
                </div>
                ...
                <div className="space-y-1.5">
                  ...
                  <div className="relative group">
                    ...
                    <select
                      ...
                      onChange={(e) =>
                        setEditingEmployee({ ...editingEmployee, companyId: e.target.value })
                      }
                    >
                      {allCompanies.map((c) => (
                        <option key={c.id} value={c.id}>
                          {c.entityName} ({c.country})
                        </option>
                      ))}
                    </select>
                  </div>
                ...
                <InputField
                  ...
                  onChange={(v) => setEditingEmployee({ ...editingEmployee, position: v })}
                />
                <InputField
                  label="Département"
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div className="space-y-1.5">
                    ...
                    <select
                      ...
                      onChange={(e) =>
                        setEditingEmployee({ ...editingEmployee, contractType: e.target.value })
                      }
                    >
                ...
                <div className="pt-4 mt-4 border-t border-slate-100">
                  <div className="flex items-center gap-2 text-sky-600 mb-4 font-bold text-xs uppercase tracking-widest">
                    <Wallet size={14} /> Rémunération
                  </div>
                  <InputField
                    ...
                    onChange={(v) =>
                      setEditingEmployee({ ...editingEmployee, baseSalary: parseFloat(v) || 0 })
                    }
                    icon={<Wallet size={16} />}
                  />
                </div>
      ...
      <Modal
        ...
        footer={
          <div className="flex gap-3 justify-end w-full">
            <button
              onClick={() => setGeneratingPayslip(null)}
              className="px-6 py-2.5 text-slate-500 font-bold hover:bg-slate-100 rounded-sm"
            >
              Annuler
            </button>
            <button
              onClick={handleGeneratePayslip}
              className="px-8 py-2.5 bg-sky-600 text-white font-black rounded-sm hover:bg-sky-700 shadow-lg shadow-sky-600/30"
            >
              Générer le Bulletin
            </button>
          </div>
          ...
          <div className="space-y-6 py-4">
            <div className="grid grid-cols-2 gap-4">
              <InputField
                ...
                value={generatingPayslip.period || ''}
                onChange={(v) => setGeneratingPayslip({ ...generatingPayslip, period: v })}
              />
              <div className="space-y-1.5">
                <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">
                  Statut Initial
                ...
                <select
                  className="w-full px-4 py-2.5 rounded-sm border border-slate-200 bg-slate-50 text-sm font-bold outline-none"
                  value={generatingPayslip.status}
                  onChange={(e) =>
                    setGeneratingPayslip({ ...generatingPayslip, status: e.target.value as any })
                  }
                >
            ...
            <div className="grid grid-cols-2 gap-8 pt-4">
              <div className="space-y-4">
                <p className="text-[10px] font-black text-green-600 uppercase tracking-widest border-b border-green-100 pb-1">
                  Gains (+){' '}
                </p>
                <InputField
                  label="Salaire de Base"
                  type="number"
                  value={generatingPayslip.baseSalary?.toString()}
                  onChange={(v) =>
                    setGeneratingPayslip({ ...generatingPayslip, baseSalary: parseFloat(v) })
                  }
                />
                <InputField
                  label="Indemnité Transport"
                  type="number"
                  value={generatingPayslip.transportPrime?.toString()}
                  onChange={(v) =>
                    setGeneratingPayslip({
                      ...generatingPayslip,
                      transportPrime: parseFloat(v) || 0,
                    })
                  }
                />
                <InputField
                  ...
                  type="number"
                  value={generatingPayslip.housingPrime?.toString()}
                  onChange={(v) =>
                    setGeneratingPayslip({ ...generatingPayslip, housingPrime: parseFloat(v) || 0 })
                  }
                />
                <InputField
                  label="Autres Bonus"
                  type="number"
                  value={generatingPayslip.otherBonuses?.toString()}
                  onChange={(v) =>
                    setGeneratingPayslip({ ...generatingPayslip, otherBonuses: parseFloat(v) || 0 })
                  }
                />
              </div>
              <div className="space-y-4">
                <p className="text-[10px] font-black text-red-600 uppercase tracking-widest border-b border-red-100 pb-1">
                  Retenues (-){' '}
                </p>
                <InputField
                  label="Part Ouvrière CNPS"
                  type="number"
                  value={generatingPayslip.cnpsDeduction?.toString()}
                  onChange={(v) =>
                    setGeneratingPayslip({
                      ...generatingPayslip,
                      cnpsDeduction: parseFloat(v) || 0,
                    })
                  }
                />
                <InputField
                  label="Impôt (IOTS)"
                  type="number"
                  value={generatingPayslip.taxDeduction?.toString()}
                  onChange={(v) =>
                    setGeneratingPayslip({ ...generatingPayslip, taxDeduction: parseFloat(v) || 0 })
                  }
                />
                <InputField
                  label="Autres Retenues"
                  type="number"
                  value={generatingPayslip.otherDeductions?.toString()}
                  onChange={(v) =>
                    setGeneratingPayslip({
                      ...generatingPayslip,
                      otherDeductions: parseFloat(v) || 0,
                    })
                  }
                />
              </div>
            ...
            <div className="space-y-4 pt-4 border-t border-slate-100">
              <p className="text-[10px] font-black text-sky-600 uppercase tracking-widest border-b border-sky-100 pb-1">
                Part Patronale (Employeur)
              </p>
              <div className="grid grid-cols-2 gap-8">
                <InputField
                  label="CNPS Part Patronale"
                  type="number"
                  placeholder="Laissé vide pour calcul auto (7.7%)"
                  value={generatingPayslip.cnpsEmployer?.toString()}
                  onChange={(v) =>
                    setGeneratingPayslip({ ...generatingPayslip, cnpsEmployer: parseFloat(v) || 0 })
                  }
                />
                <InputField
                  label="Taxe Patronale / FDFP"
                  type="number"
                  placeholder="Laissé vide pour calcul auto (1.5%)"
                  value={generatingPayslip.taxEmployer?.toString()}
                  onChange={(v) =>
                    setGeneratingPayslip({ ...generatingPayslip, taxEmployer: parseFloat(v) || 0 })
                  }
                />
              </div>
            </div>
          ...
          <div className="p-4 space-y-8">
                ...
                <div className="bg-slate-50 p-6 rounded-sm border border-slate-100">
                  ...
                  <div className="text-xs text-slate-500 mt-2 space-y-1">
                    <p className="font-bold uppercase">
                      Matricule:{' '}
                      {employees.find((e) => e.id === showPayslipPreview.employeeId)?.matricule}
                    </p>
                    <p>
                      Poste:{' '}
                      {employees.find((e) => e.id === showPayslipPreview.employeeId)?.position}
                    </p>
                    <p>
                      Entrée:{' '}
                      {new Date(
                        employees.find((e) => e.id === showPayslipPreview.employeeId)?.joinDate ||
                        '',
                      ).toLocaleDateString()}
                    </p>
                  </div>
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="flex justify-between items-center text-xs">
                      ...
                      <span className="font-black text-red-600">
                        -{' '}
                        {(
                          showPayslipPreview.grossSalary - showPayslipPreview.netSalary
                        ).toLocaleString()}{' '}
                        CFA
                      </span>
                    </div>
            ...
            <div className="flex gap-4 justify-center no-print">
              <PDFDownloadLink
                document={
                  <PayslipPDF
                    payslip={showPayslipPreview}
                    employee={
                      employees.find((e) => e.id === showPayslipPreview.employeeId) ||
                      ({} as Employee)
                    }
                  />
                }
              ...
              <button
                ...
                className="flex items-center gap-2 px-8 py-3 bg-slate-800 text-white rounded-sm hover:bg-slate-900 font-bold transition-all"
              >
          ...
          <button
            onClick={() => {
              createAttendance({
                ...newAttendance,
                employeeName:
                  employees.find((e: any) => e.id === newAttendance.employeeId)?.fullName ||
                  'Inconnu',
            ...
            className="px-8 py-3 bg-slate-900 text-white font-black rounded-sm shadow-lg hover:shadow-xl transition-all active:scale-95"
          >
            Pointer Présence
          </button>
        }
      >
        <div className="space-y-4 py-2">
          ...
          <InputField
            ...
            onChange={(v) => setNewAttendance({ ...newAttendance, date: v })}
          />
          <div className="grid grid-cols-2 gap-4">
            <InputField
              ...
              onChange={(v) => setNewAttendance({ ...newAttendance, checkIn: v })}
            />
            <InputField
              label="Heure de départ"
              type="time"
              value={newAttendance.checkOut}
              onChange={(v) => setNewAttendance({ ...newAttendance, checkOut: v })}
            />
          </div>
          <div className="space-y-1.5">
            <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">
          ...
          <button
            onClick={() => {
              createLeaveRequest({
                ...newLeave,
                employeeName:
                  employees.find((e: any) => e.id === newLeave.employeeId)?.fullName || 'Inconnu',
              });
              setShowingLeaveModal(false);
            }}
            className="px-8 py-3 bg-sky-600 text-white font-black rounded-sm shadow-lg shadow-sky-600/30 hover:bg-sky-700 transition-all active:scale-95"
          >
        ...
        <div className="grid grid-cols-2 gap-6 py-2">
          ...
          <div className="space-y-4">
            <InputField
              ...
              onChange={(v) => setNewLeave({ ...newLeave, startDate: v })}
            />
            <InputField
              label="Date de fin"
              type="date"
              value={newLeave.endDate}
              onChange={(v) => setNewLeave({ ...newLeave, endDate: v })}
            />
          </div>
          <div className="col-span-2 space-y-1.5">
            ...
            <textarea
              className="w-full p-4 rounded-sm border border-slate-200 bg-slate-50 text-sm font-medium outline-none focus:bg-white focus:ring-4 focus:ring-sky-500/10 transition-all shadow-inner"
              rows={3}
