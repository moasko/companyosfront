...
export const FaceTrainingModal: React.FC<FaceTrainingModalProps> = ({
  isOpen,
  onClose,
  employee,
  onSave,
  ...
  const [status, setStatus] = useState<'loading' | 'ready' | 'detecting' | 'captured' | 'error'>(
    'loading',
  );
    ...
    >
      <div className="flex flex-col items-center gap-6 py-4">
        ...
        <div className="text-center space-y-4 w-full max-w-md">
          <div
            className={`p-4 rounded-lg flex items-center justify-center gap-3 ${status === 'error' ? 'bg-red-50 text-red-600' : 'bg-slate-50 text-slate-600'}`}
          >
        ...
        <p className="text-xs text-slate-400 text-center max-w-xs">
          L'empreinte faciale est convertie en une signature mathématique chiffrée. Aucune image
          n'est conservée.
        </p>
      </div>
    </Modal>
