...
import {
  ...
  InputField,
  ImageUpload,
} from '@/components/admin/shared/AdminShared';
import {
  Building2,
  Plus,
  Settings,
    ...
    <div className="space-y-8 animate-in fade-in duration-500">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        <div className="bg-white p-6 rounded-sm border border-slate-200 shadow-sm">
          ...
          <div className="flex items-end gap-2">
            ...
            <span className="text-[10px] font-bold text-green-600 mb-1.5 flex items-center gap-1 transition-transform group-hover:translate-x-1">
              Filiales
            </span>
          </div>
        </div>
        <div className="bg-white p-6 rounded-sm border border-slate-200 shadow-sm">
          ...
          <div className="flex items-end gap-2">
            ...
            <span className="text-[10px] font-bold text-sky-600 mb-1.5 uppercase">
              Collaborateurs
            </span>
          </div>
        </div>
        <div className="bg-white p-6 rounded-sm border border-slate-200 shadow-sm">
          ...
          <div className="flex items-end gap-2">
            ...
            <span className="text-[10px] font-bold text-amber-600 mb-1.5 uppercase">
              Références
            </span>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
        {filteredCompanies.map((company) => (
          <div
            key={company.id}
            className="bg-white rounded-sm border border-slate-200 shadow-sm hover:shadow-2xl hover:-translate-y-1 transition-all group relative overflow-hidden flex flex-col"
          >
            <div
              className="h-2"
              style={{ backgroundColor: company.primaryColor || '#0ea5e9' }}
            ></div>
                ...
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <div className="flex items-center gap-2">
                      <Badge
                        color="slate"
                        className="text-[9px] font-black uppercase tracking-tighter px-2"
                      >
                        {company.legalForm}
                      </Badge>
        ...
        footer={
          <div className="flex gap-3 justify-end w-full">
            <button
              ...
              className="px-6 py-2.5 text-slate-600 font-semibold hover:bg-slate-100 rounded-sm transition-all text-xs uppercase tracking-widest"
            >
              Annuler
            </button>
            <button
              onClick={handleSave}
              className="px-8 py-2.5 bg-sky-600 text-white font-black rounded-sm hover:bg-sky-700 shadow-xl shadow-sky-600/20 transition-all flex items-center gap-2 text-xs uppercase tracking-widest active:scale-95"
            >
              Mettre à jour
            </button>
          </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="space-y-4">
                  <InputField
                    ...
                    value={isEditing.entityName || ''}
                    onChange={(v) => setIsEditing({ ...isEditing, entityName: v })}
                  />
                  <InputField
                    label="Activité Principale"
                    value={isEditing.activity || ''}
                    onChange={(v) => setIsEditing({ ...isEditing, activity: v })}
                  />
                  ...
                  <div className="space-y-1.5">
                    ...
                    <div className="flex flex-wrap gap-2">
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          setIsEditing({ ...isEditing, primaryColor: e.target.value })
                        }
                ...
                <div className="space-y-4">
                  ...
                  <InputField
                    ...
                    onChange={(v) => setIsEditing({ ...isEditing, slug: v })}
                  />
                </div>
              </div>
            </div>
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <InputField
                  ...
                  value={isEditing.capital || ''}
                  onChange={(v) => setIsEditing({ ...isEditing, capital: v })}
                />
                <InputField
                  label="Gérant / DG"
                  value={isEditing.manager || ''}
                  onChange={(v) => setIsEditing({ ...isEditing, manager: v })}
                />
                <InputField
                  label="Numéro NCC"
                  value={isEditing.ncc || ''}
                  onChange={(v) => setIsEditing({ ...isEditing, ncc: v })}
                />
                <InputField
                  label="Numéro RCCM"
                  value={isEditing.rccm || ''}
                  onChange={(v) => setIsEditing({ ...isEditing, rccm: v })}
                />
                <InputField
                  label="Taux TVA (%)"
                  type="number"
                  value={isEditing.tvaRate?.toString() || '18'}
                  onChange={(v) => setIsEditing({ ...isEditing, tvaRate: parseFloat(v) })}
                />
              </div>
            </div>
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <InputField
                  ...
                  onChange={(v) => setIsEditing({ ...isEditing, address: v })}
                />
                <div className="grid grid-cols-2 gap-4">
                  <InputField
                    label="Ville"
                    value={isEditing.city || ''}
                    onChange={(v) => setIsEditing({ ...isEditing, city: v })}
                  />
                  <InputField
                    label="Boîte Postale"
                    value={isEditing.bp || ''}
                    onChange={(v) => setIsEditing({ ...isEditing, bp: v })}
                  />
                </div>
                <InputField
                  label="Téléphone"
                  value={isEditing.phone || ''}
                  onChange={(v) => setIsEditing({ ...isEditing, phone: v })}
                />
                <InputField
                  label="Email"
                  type="email"
                  value={isEditing.email || ''}
                  onChange={(v) => setIsEditing({ ...isEditing, email: v })}
                />
              </div>
            </div>
          </div>
        ...
        footer={
          <div className="flex gap-3 justify-end w-full">
            ...
            <button
              ...
              className="px-8 py-2.5 bg-slate-900 text-white font-bold rounded-sm hover:bg-black shadow-lg transition-all flex items-center gap-2"
            >
              Créer l'entité
            </button>
          </div>
      ...
      >
        <div className="space-y-8 py-2 max-h-[70vh] overflow-y-auto px-1 custom-scrollbar">
          {/* Identité */}
          <div className="space-y-4">
            <h4 className="flex items-center gap-2 text-slate-800 font-black text-sm uppercase tracking-wider pb-2 border-b border-slate-100">
              Identité Visuelle
            </h4>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <InputField
                label="URL Slug (Auto)"
                value={newData.slug}
                onChange={(v) => setNewData({ ...newData, slug: v })}
              />
              <InputField
                label="Activité"
                placeholder="Ex: Import-Export"
                value={newData.activity}
                onChange={(v) => setNewData({ ...newData, activity: v })}
              />
              <div className="space-y-1.5">
                <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest ml-1">
                  Secteur
            ...
            <ImageUpload
              ...
              value={newData.logo}
              onChange={(url) => setNewData({ ...newData, logo: url })}
            />
          </div>
          ...
          <div className="space-y-4 border-t border-slate-100 pt-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <InputField
                label="Gérant / Représentant"
                value={newData.manager}
                onChange={(v) => setNewData({ ...newData, manager: v })}
              />
            </div>
          </div>
          ...
          <div className="p-4 bg-sky-50 rounded-sm border border-sky-100 flex items-start gap-3">
            ...
            <div>
              <p className="text-xs font-bold text-sky-800">
                Un nouveau site web sera généré automatiquement.
              </p>
              <p className="text-[10px] text-sky-600 font-medium">
                L'adresse de votre portail sera : enea-group.com/{newData.slug || 'nom-entite'}
              </p>
            </div>
          </div>
        </div>
