...
import { Building2, Plus, Globe, Tag, ChevronRight, Layout, Check, Sparkles, ArrowRight } from 'lucide-react';
            ...
            {step === 0 && (
                <div className="max-w-2xl w-full bg-white rounded-lg shadow-2xl overflow-hidden animate-in zoom-in-95 duration-500 relative">
                    ...
                    <div className="p-12 text-center">
                        ...
                        <h1 className="text-4xl font-black text-slate-900 mb-4 tracking-tight">Bienvenue sur ENEA.</h1>
                        <p className="text-slate-500 text-lg mb-8 leading-relaxed max-w-lg mx-auto">
                            Votre plateforme de gestion unifiée est prête. <br />Nous allons configurer ensemble votre premier espace de travail numérique.
                        </p>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-12 text-left">
                            <div className="p-4 bg-slate-50 rounded border border-slate-100">
                                ...
                                <p className="text-xs text-slate-500">Gestion de stock, RH, Finances centralisée.</p>
                            </div>
                            <div className="p-4 bg-slate-50 rounded border border-slate-100">
                                <Globe className="text-indigo-500 mb-3" size={24} />
                                <h3 className="font-bold text-slate-800 mb-1">Site Web</h3>
                                <p className="text-xs text-slate-500">Votre vitrine en ligne générée automatiquement.</p>
                            </div>
                            <div className="p-4 bg-slate-50 rounded border border-slate-100">
                        ...
                        <div className="flex flex-col gap-4">
                            ...
                            <button onClick={onLogout} className="text-slate-400 text-xs font-bold hover:text-slate-600 uppercase tracking-widest">
                                Se déconnecter
                            </button>
                        </div>
                    </div>
                ...
                <div className="max-w-6xl w-full grid grid-cols-1 lg:grid-cols-12 bg-white rounded-lg shadow-2xl overflow-hidden animate-in fade-in slide-in-from-bottom-8 duration-500 h-[85vh]">
                    ...
                    <div className="lg:col-span-4 bg-slate-50 border-r border-slate-100 p-8 flex flex-col">
                        <div className="mb-10">
                            ...
                            <span className="text-slate-400 text-xs ml-2 uppercase font-bold tracking-widest">Setup</span>
                        </div>
                        ...
                        <div className="flex-1 space-y-8">
                            {[
                                ...
                                <div key={s.id} className={`flex items-start gap-4 transition-all duration-300 ${step === s.id ? 'opacity-100 translate-x-2' : step > s.id ? 'opacity-50' : 'opacity-30'}`}>
                                    <div className={`w-10 h-10 rounded-full flex items-center justify-center shrink-0 border-2 ${step > s.id ? 'bg-green-500 border-green-500 text-white' : step === s.id ? 'bg-white border-sky-600 text-sky-600 shadow-lg shadow-sky-100' : 'bg-transparent border-slate-300 text-slate-300'}`}>
                                        {step > s.id ? <Check size={18} strokeWidth={4} /> : <s.icon size={18} />}
                                    </div>
                                    <div className="pt-1">
                                        <h4 className={`font-bold text-sm ${step === s.id ? 'text-sky-900' : 'text-slate-600'}`}>{s.title}</h4>
                        ...
                        <div className="mt-auto">
                            <button onClick={onLogout} className="text-xs text-slate-400 font-bold hover:text-red-500 transition-colors flex items-center gap-2">
                        ...
                        <div className="max-w-2xl mx-auto h-full flex flex-col justify-center">
                            ...
                            <form onSubmit={handleSubmit} className="space-y-6">
                                    ...
                                    <div className="space-y-6 animate-in fade-in slide-in-from-right-8 duration-300">
                                        ...
                                        <div className="p-4 bg-sky-50 rounded border border-sky-100 flex items-start gap-3">
                                            ...
                                            <div>
                                                ...
                                                <p className="text-xs text-sky-700 mt-1 mb-2">Votre site sera accessible via ce lien unique.</p>
                                        ...
                                        <button onClick={() => formData.entityName && setStep(2)} type="button" className="w-full bg-slate-900 text-white py-4 rounded font-bold uppercase tracking-widest hover:bg-black transition-all flex justify-between px-6 items-center group">
                                            Suivant <ArrowRight className="group-hover:translate-x-1 transition-transform" />
                                        </button>
                                    </div>
                                )}
                                ...
                                {step === 2 && (
                                    <div className="space-y-6 animate-in fade-in slide-in-from-right-8 duration-300">
                                        <div className="grid grid-cols-2 gap-6">
                                            ...
                                            <div className="space-y-1.5">
                                                <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest ml-1">Pays</label>
                                                <select
                                                    className="w-full bg-slate-50 border-slate-200 px-4 py-3 rounded border focus:ring-2 focus:ring-sky-500 focus:border-sky-500 outline-none transition-all font-medium text-slate-700 text-sm"
                                                    value={formData.country}
                                                    onChange={(e) => {
                                                        const flags: any = { "Côte d'Ivoire": "🇨🇮", "Sénégal": "🇸🇳", "Burkina Faso": "🇧🇫", "France": "🇫🇷", "Mali": "🇲🇱", "Cameroun": "🇨🇲" };
                                                        setFormData({ ...formData, country: e.target.value, flag: flags[e.target.value] || '🌍' });
                                                    }}
                                                >
                                        ...
                                        <div className="grid grid-cols-2 gap-6 pt-4 border-t border-slate-100">
                                            <InputField label="Numéro NCC" value={formData.ncc} onChange={(v) => setFormData({ ...formData, ncc: v })} placeholder="Facultatif" />
                                            <InputField label="Numéro RCCM" value={formData.rccm} onChange={(v) => setFormData({ ...formData, rccm: v })} placeholder="Facultatif" />
                                        </div>

                                        <div className="flex gap-4 pt-4">
                                            <button onClick={() => setStep(1)} type="button" className="px-6 py-4 rounded text-slate-500 font-bold uppercase tracking-widest hover:bg-slate-50 transition-colors">Retour</button>
                                            <button onClick={() => setStep(3)} type="button" className="flex-1 bg-slate-900 text-white py-4 rounded font-bold uppercase tracking-widest hover:bg-black transition-all flex justify-between px-6 items-center group">
                                                Suivant <ArrowRight className="group-hover:translate-x-1 transition-transform" />
                                            </button>
                                        </div>
                                    </div>
                                ...
                                {step === 3 && (
                                    <div className="space-y-6 animate-in fade-in slide-in-from-right-8 duration-300">
                                        <div className="grid grid-cols-2 gap-6">
                                            <InputField label="Email Contact" value={formData.email} onChange={(v) => setFormData({ ...formData, email: v })} type="email" placeholder="contact@..." />
                                            <InputField label="Téléphone" value={formData.phone} onChange={(v) => setFormData({ ...formData, phone: v })} placeholder="+XXX XX XX XX XX" />
                                        </div>

                                        <InputField label="Adresse Postale" value={formData.address} onChange={(v) => setFormData({ ...formData, address: v })} placeholder="Ville, Quartier, Rue..." />

                                        <div className="space-y-3 pt-4">
                                            <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest ml-1">Couleur Principale</label>
                                            <div className="flex gap-3">
                                                {['#0ea5e9', '#6366f1', '#8b5cf6', '#ec4899', '#ef4444', '#f59e0b', '#10b981', '#1e293b'].map((color) => (
                                                    <button
                                                        key={color}
                                                        type="button"
                                        ...
                                        <div className="flex gap-4 pt-6">
                                            <button onClick={() => setStep(2)} type="button" className="px-6 py-4 rounded text-slate-500 font-bold uppercase tracking-widest hover:bg-slate-50 transition-colors">Retour</button>
                                            <button type="submit" className="flex-1 bg-green-600 text-white py-4 rounded font-bold uppercase tracking-widest hover:bg-green-700 transition-all flex justify-center gap-3 items-center shadow-lg shadow-green-200">
            ...
            {step === 4 && (
                <div className="max-w-md w-full bg-white rounded-lg shadow-2xl p-12 text-center animate-in zoom-in-95 duration-500">
                    <div className="relative w-24 h-24 mx-auto mb-8">
                        {loadingState < 4 && <div className="absolute inset-0 border-4 border-slate-100 rounded-full"></div>}
                        {loadingState < 4 && <div className="absolute inset-0 border-4 border-sky-500 border-t-transparent rounded-full animate-spin"></div>}
                        {loadingState === 4 && (
                    ...
                    <div className="space-y-4 text-left max-w-xs mx-auto">
                        <div className={`flex items-center gap-3 transition-all duration-500 ${loadingState >= 1 ? 'opacity-100' : 'opacity-30'}`}>
                            <div className={`w-5 h-5 rounded-full flex items-center justify-center text-[10px] ${loadingState >= 1 ? 'bg-green-500 text-white' : 'bg-slate-200'}`}>
                                {loadingState >= 1 && <Check size={12} />}
                            </div>
                            <span className="text-sm font-medium text-slate-700">Création de la base de données</span>
                        </div>
                        <div className={`flex items-center gap-3 transition-all duration-500 delay-100 ${loadingState >= 2 ? 'opacity-100' : 'opacity-30'}`}>
                            <div className={`w-5 h-5 rounded-full flex items-center justify-center text-[10px] ${loadingState >= 2 ? 'bg-green-500 text-white' : 'bg-slate-200'}`}>
                                {loadingState >= 2 && <Check size={12} />}
                            </div>
                            <span className="text-sm font-medium text-slate-700">Configuration des modules</span>
                        </div>
                        <div className={`flex items-center gap-3 transition-all duration-500 delay-200 ${loadingState >= 3 ? 'opacity-100' : 'opacity-30'}`}>
                            <div className={`w-5 h-5 rounded-full flex items-center justify-center text-[10px] ${loadingState >= 3 ? 'bg-green-500 text-white' : 'bg-slate-200'}`}>
                                {loadingState >= 3 && <Check size={12} />}
                            </div>
                            <span className="text-sm font-medium text-slate-700">Génération du site web</span>
                        </div>
                        <div className={`flex items-center gap-3 transition-all duration-500 delay-300 ${loadingState >= 4 ? 'opacity-100' : 'opacity-30'}`}>
                            <div className={`w-5 h-5 rounded-full flex items-center justify-center text-[10px] ${loadingState >= 4 ? 'bg-green-500 text-white' : 'bg-slate-200'}`}>
                                {loadingState >= 4 && <Check size={12} />}
                            </div>
                            <span className="text-sm font-medium text-slate-700">Initialisation des données exemples</span>
                        </div>
                    </div>
                </div>
            )}
