...
export const TasksModule: React.FC<TasksModuleProps> = ({ companyId }) => {
  ...
  const {
    ...
    updateTask,
    deleteTask,
  } = useTasks(companyId, { year: selectedYear, month: selectedMonth });
          ...
          <div className="flex gap-3">
            <div className="flex bg-slate-100 p-1 rounded-sm border border-slate-200">
              <button
                ...
                className={`p-2 rounded-sm transition-all ${viewMode === 'board' ? 'bg-white shadow-sm text-sky-600' : 'text-slate-400 hover:text-slate-600'}`}
              >
                <LayoutGrid size={18} />
              </button>
              <button
                onClick={() => setViewMode('list')}
                className={`p-2 rounded-sm transition-all ${viewMode === 'list' ? 'bg-white shadow-sm text-sky-600' : 'text-slate-400 hover:text-slate-600'}`}
              >
                <List size={18} />
              </button>
            </div>
        ...
        <DataTable<Task>
          ...
          columns={[
              ...
              accessor: (t) => (
                <div className="py-1">
                  <div className="flex items-center gap-2">
                    ...
                    {t.category && (
                      <span className="text-[8px] font-black bg-slate-100 text-slate-500 px-1 rounded uppercase tracking-tighter">
                        {t.category}
                      </span>
                    )}
                  </div>
                  ...
                  {t.tags && t.tags.length > 0 && (
                    <div className="flex gap-1 mt-1">
                      {t.tags.slice(0, 3).map((tag) => (
                        <span key={tag} className="text-[8px] text-slate-400">
                          #{tag}
                        </span>
                      ))}
                    </div>
            ...
            },
            {
              ...
              accessor: (t) => (
                <div
                  className={`flex items-center gap-1.5 text-xs font-bold ${t.dueDate && new Date(t.dueDate) < new Date() && t.status !== 'Done' ? 'text-red-500' : 'text-slate-500'}`}
                >
                  <Clock size={12} />
            ...
            },
            {
              ...
              accessor: (t) => (
                <div className="flex flex-col gap-1">
                  ...
                  {!t.clientName && !t.supplierName && !t.productName && (
