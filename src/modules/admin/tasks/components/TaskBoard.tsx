...
const Column = ({
  ...
  color: string;
  onEdit: (t: Task) => void;
  onStatusChange: (t: Task, s: any) => void;
}) => {
  return (
    <div className="flex-1 min-w-[300px] flex flex-col bg-slate-100/50 rounded-sm border border-slate-200/60 h-full max-h-full">
      <div className={`p-4 border-b border-slate-200 flex justify-between items-center ${color}`}>
        <h3 className="font-black text-slate-700 uppercase tracking-tight text-sm flex items-center gap-2">
          <div
            className={`w-2 h-2 rounded-full ${status === 'Todo' ? 'bg-slate-400' : status === 'In Progress' ? 'bg-blue-500' : status === 'Done' ? 'bg-green-500' : 'bg-purple-500'}`}
          ></div>
          {title}
        </h3>
        <span className="bg-white px-2 py-0.5 rounded-full text-[10px] font-bold text-slate-500 shadow-sm border border-slate-100">
          {tasks.length}
        </span>
      </div>
      <div className="p-3 space-y-3 overflow-y-auto flex-1 custom-scrollbar">
        {tasks.map((task) => (
          <div
            key={task.id}
            className="bg-white p-4 rounded-sm border border-slate-200 shadow-sm hover:shadow-md transition-all group cursor-pointer relative"
            onClick={() => onEdit(task)}
          >
            ...
            <div className="flex justify-between items-start mb-2">
              ...
              {task.dueDate && (
                <div
                  className={`flex items-center gap-1 text-[10px] font-bold ${new Date(task.dueDate) < new Date() && status !== 'Done' ? 'text-red-500' : 'text-slate-400'}`}
                >
                  <Clock size={12} />
              ...
              <div className="flex gap-1" onClick={(e) => e.stopPropagation()}>
                {status !== 'Todo' && (
                  <button
                    ...
                    className="p-1 hover:bg-slate-100 rounded text-slate-400 hover:text-slate-600 text-[10px] font-bold"
                  >
                    ←
                  </button>
                )}
