...
export const InvoiceModal: React.FC<InvoiceModalProps> = ({
  ...
  const updateItem = (idx: number, field: keyof InvoiceItem, val: any) => {
    ...
    if (field === 'quantity' || field === 'unitPrice') {
      const quantity = field === 'quantity' ? Number(val) : Number(items[idx].quantity);
      const unitPrice = field === 'unitPrice' ? Number(val) : Number(items[idx].unitPrice);
      // Ensure we handle floating point issues and rounding
      items[idx].total = Math.round(quantity * unitPrice);
    }
  ...
  return (
    <Modal
      ...
      onClose={onClose}
      title={
        invoice.id?.startsWith('new-')
          ? 'Créer une nouvelle facture'
          : `Modifier la facture ${invoice.reference}`
      }
      size="xl"
      footer={
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 bg-slate-50 p-6 rounded-sm border border-slate-100">
          <div className="space-y-1">
            <InputField
              ...
              onChange={(v) => onChange({ ...invoice, reference: v })}
