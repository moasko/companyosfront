...
export const QuoteModal: React.FC<QuoteModalProps> = ({ isOpen, onClose, quote, stock, contacts, onSave, onChange }) => {
    if (!quote) return null;

    const addItem = () => {
        const newItem: QuoteItem = { id: 'new-' + Date.now(), description: '', quantity: 1, unitPrice: 0, total: 0 };
    ...
    return (
        <Modal
            isOpen={isOpen}
            onClose={onClose}
            title={quote.id?.startsWith('new-') ? "Créer un nouveau devis" : `Modifier le devis ${quote.reference}`}
            size="xl"
            footer={
            ...
            <div className="space-y-8">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6 bg-slate-50 p-6 rounded-sm border border-slate-100">
                    <div className="space-y-1">
                        <InputField label="Référence de offre" value={quote.reference} onChange={v => onChange({ ...quote, reference: v })} placeholder="Ex: DEV-2024-001" />
                    </div>
                    ...
                    <div className="space-y-1">
                        <InputField type="select" label="Statut commercial" value={quote.status} onChange={v => onChange({ ...quote, status: v as any })} options={[
                    ...
                    <InputField type="date" label="Date d'émission" value={quote.date ? new Date(quote.date).toISOString().split('T')[0] : ''} onChange={v => onChange({ ...quote, date: v })} />
                    <InputField type="date" label="Date d'expiration" value={quote.validUntil ? new Date(quote.validUntil).toISOString().split('T')[0] : ''} onChange={v => onChange({ ...quote, validUntil: v })} />
                ...
                <div className="space-y-4">
                                        ...
                                        <td className="p-4">
                                            <div className="space-y-2">
                                                <div className="flex gap-2">
                                                    <div className="mt-2.5 text-slate-300"><ShoppingBag size={14} /></div>
                                                    <select
                                                    ...
                                                    >
                                                        ...
                                                        {stock.map(s => <option key={s.id} value={s.id}>{s.name} ({s.quantity} {s.unit} en stock)</option>)}
                                                    </select>
                                                </div>
                                                <input
                                    ...
                                    <tr>
                                        <td colSpan={5} className="p-12 text-center">
                                            <div className="flex flex-col items-center gap-2 opacity-30">
                                                ...
                                                <p className="text-sm italic font-medium">Aucun article dans ce devis. Commencez par en ajouter un.</p>
                                            </div>
                                        </td>
                                    </tr>
                                )}
                    ...
                    <div className="flex flex-col md:flex-row justify-between items-start gap-8 pt-4">
                        <div className="flex-1 w-full space-y-2">
                            <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Notes internes & Observations</label>
                            <textarea
                                ...
                                <div className="relative z-10 space-y-4">
                                    ...
                                    <div className="pt-4 border-t border-white/10 flex justify-between items-end">
                                        <div>
                                            ...
                                            <div className="text-4xl font-black">{quote.totalAmount.toLocaleString()}</div>
                                        </div>
