...
export const useSettings = (companyId: string) => {
    ...
    const createMutation = useMutation({
        mutationFn: (data: Partial<DictionaryItem>) => apiFetch(`/settings/${companyId}`, { method: 'POST', body: JSON.stringify(data) }),
        onSuccess: () => queryClient.invalidateQueries({ queryKey: ['settings', companyId] })
    });

    const updateMutation = useMutation({
        mutationFn: ({ id, ...data }: any) => apiFetch(`/settings/${id}`, { method: 'PATCH', body: JSON.stringify(data) }),
        onSuccess: () => queryClient.invalidateQueries({ queryKey: ['settings', companyId] })
    });
    ...
    return {
        ...
        savePrinterSettings: (data: any) => apiFetch(`/settings/${companyId}/printers`, { method: 'POST', body: JSON.stringify(data) }),
