...
export class SettingsService {
    ...
    async create(companyId: string, data: { type: string; value: string; code?: string; color?: string }) {
    ...
    async discoverPrinters() {
        ...
        try {
            ...
            const { stdout } = await execAsync('powershell -Command "Get-Printer | Select-Object Name, PortName, DriverName, PrinterStatus | ConvertTo-Json"');

            if (stdout && stdout.trim()) {
                const psPrinters = JSON.parse(stdout);
                const list = Array.isArray(psPrinters) ? psPrinters : [psPrinters];
                printers.push(...list.map(p => ({
                    name: p.Name,
                    type: 'INSTALLED',
